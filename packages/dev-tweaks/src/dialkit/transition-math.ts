...
import { isEasingConfigValue, isSpringConfigValue } from "./store/dial-store";
...
export function springParams(spring: SpringConfig): SpringParams {
  ...
  const damping =
    2 * Math.min(1, Math.max(0.05, 1 - bounce)) * Math.sqrt(stiffness);
