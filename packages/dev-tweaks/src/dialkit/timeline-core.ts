...
function collectClipEntries(config: TimelineConfig): ClipParseEntry[] {
  ...
  for (const [key, value] of Object.entries(config)) {
    ...
    if (RESERVED_KEYS.has(key)) {
      console.warn(
        `[dialkit] Timeline key "${key}" collides with a reserved key and was skipped.`
      );
      continue;
    }
    ...
    if (clip.props) {
      ...
      for (const [prop, track] of Object.entries(clip.props)) {
        if (TRACK_RESERVED.has(prop) || STEP_PROP_KEY_REGEX.test(prop)) {
          console.warn(
            `[dialkit] Timeline property "${prop}" collides with a clip field and was skipped.`
          );
          continue;
        }
...
function scalarDial(
  ...
  const record = withFromToRanges(
    ...
    counterpart === undefined
      ? undefined
      : ({ [prop]: counterpart } as DialConfig)
  );
...
function withFromToRanges(
  config: DialConfig,
  counterpart: DialConfig | undefined
): DialConfig {
  const result: DialConfig = {};
  for (const [key, value] of Object.entries(config)) {
...
export function computeStaticClips(
  ...
  const resolved = resolveDialValues(parsed.dialConfig, flatValues) as Record<
    string,
    unknown
  >;
