...
export type DialKitPersistOptions =
  | boolean
  | {
      key?: string;
...
export interface DialStorePanelOptions {
  kind?: "timeline";
...
class DialStoreClass {
  ...
  private readonly snapshots: Map<string, Record<string, DialValue>> =
    new Map();
  ...
  private readonly baseValues: Map<string, Record<string, DialValue>> =
    new Map();
  private readonly defaultValues: Map<string, Record<string, DialValue>> =
    new Map();
  ...
  private normalizePersistConfig(
    id: string,
    persist: DialKitPersistOptions | undefined
  ): PersistConfig | null {
    if (!persist) {
      return null;
    }
