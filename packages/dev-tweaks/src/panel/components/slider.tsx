/* eslint-disable react-hooks/refs, react-hooks/set-state-in-effect -- ported panel render-update patterns kept structurally intact */
...
export function Slider({
  ...
  const hashMarks =
    discreteSteps <= 10
      ? Array.from({ length: discreteSteps - 1 }, (_, i) => {
          ...
          return (
            <div
              className="dev-tweaks-slider-hashmark"
              key={pct}
              style={{ left: `${pct}%` }}
            />
          );
  ...
  return (
    <div className="dev-tweaks-slider-wrapper" ref={wrapperRef}>
      <motion.div
        className={`dev-tweaks-slider ${isActive ? "dev-tweaks-slider-active" : ""}`}
      ...
      >
        <div className="dev-tweaks-slider-hashmarks">{hashMarks}</div>

        <motion.div
          className="dev-tweaks-slider-fill"
          style={{
            width: fillWidth,
          }}
        />
        ...
        <motion.div
          ...
          className="dev-tweaks-slider-handle"
          style={{
            left: handleLeft,
            y: "-50%",
          }}
        ...
        <span className="dev-tweaks-slider-label" ref={labelRef}>
          {label}
          {shortcut && (
            <span
        ...
        {showInput ? (
          <input
            className="dev-tweaks-slider-input"
          ...
          // biome-ignore lint/a11y/noStaticElementInteractions: hover-gated edit affordance inside the pointer-driven scrub surface; keyboard editing goes through the input
          // biome-ignore lint/a11y/noNoninteractiveElementInteractions: hover-gated edit affordance inside the pointer-driven scrub surface; keyboard editing goes through the input
          // biome-ignore lint/a11y/useKeyWithClickEvents: hover-gated edit affordance inside the pointer-driven scrub surface; keyboard editing goes through the input
          <span
            className={`dev-tweaks-slider-value ${isValueEditable ? "dev-tweaks-slider-value-editable" : ""}`}
