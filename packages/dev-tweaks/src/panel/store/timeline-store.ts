// Playback transport + registry for panel timelines.
// Clip values (at/duration/from/to) live in DevTweaksStore under the timeline's
// panel id — this store only owns time: the clock, play state, and the
// track/clip structure the dock UI renders.

export interface TimelineClipTrackMeta {
  prop: string;
...
export interface TimelineClipMeta {
  ...
  group?: string;
  key: string; // config path; also the DevTweaksStore path prefix, e.g. "cardEnter" or "circle.path"
  label: string;
...
export interface TimelineMeta {
  ...
  id: string; // same id as the DevTweaksStore panel
  loop: boolean;
...
export function loopSpan(duration: number, loopStart: number): number {
  ...
  const safeLoopStart = Number.isFinite(loopStart) ? loopStart : 0;
  const start = Math.min(Math.max(0, safeLoopStart), duration);
...
const EMPTY_TRANSPORT: TimelineTransport = Object.freeze({
  time: 0,
  playing: false,
  duration: 0,
  wraps: 0,
...
class TimelineStoreClass {
  private readonly timelines: Map<string, TimelineMeta> = new Map();
  private readonly transports: Map<string, TimelineTransport> = new Map();
  ...
  play(id: string): void {
    ...
    if (!transport || transport.duration <= 0 || transport.playing) {
      return;
    }
  ...
  seek(id: string, time: number): void {
    const transport = this.transports.get(id);
    if (!(transport && Number.isFinite(time))) {
      return;
    }
