export interface PanelDragOffset {
  x: number;
  y: number;
}

export interface PanelDragStart {
  elX: number;
  elY: number;
  pointerX: number;
  pointerY: number;
...
const DRAG_EXCLUSION_SELECTOR = [
  ".dev-tweaks-panel-icon",
  ".dev-tweaks-panel-toolbar",
  "button",
  "input",
  "select",
  "textarea",
...
export function getPanelDragHandle(
  ...
  const inner = target.closest<HTMLElement>(".dev-tweaks-panel-inner");
  ...
  const header = target.closest<HTMLElement>(".dev-tweaks-panel-header");
  if (!(header && inner.contains(header))) {
    return null;
  }

  if (target.closest(DRAG_EXCLUSION_SELECTOR)) {
    return null;
  }
...
export function getPanelDragStart(
  pointerX: number,
  pointerY: number,
  panel: HTMLElement
): PanelDragStart {
...
export function getPanelDragOffset(
...
export function hasPanelDragMoved(
  start: PanelDragStart,
  pointerX: number,
  pointerY: number
): boolean {
...
export function getPanelOriginX(
  ...
  if (offset && viewportWidth) {
    return offset.x + COLLAPSED_PANEL_SIZE / 2 < viewportWidth / 2
      ? "left"
      : "right";
  }
