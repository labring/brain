...
- Assistant chat values: `SYSTEM_OPENAI_*` funds `FREE_CHAT_TURNS` successful turns for eligible Active Free Trial workspaces; later turns use the caller's AI Proxy (`AI_PROXY_TOKEN_NAME`). `CODEX_GATEWAY_MODEL` optionally selects the Assistant chat model
