...
- optional Chat Assistant telemetry (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`); when both keys are set, Brain exports AI SDK v7 traces through OpenTelemetry. `LANGFUSE_BASE_URL` defaults to `https://cloud.langfuse.com` and may point to a self-hosted Langfuse instance. Do not append `/api/public/otel`.
