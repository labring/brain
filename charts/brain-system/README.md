...
- optional Chat Assistant Langfuse tracing (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_HOST`); all three values must be set. An empty host disables tracing; Cloud requires an explicit `https://cloud.langfuse.com`. Use an instance root URL without `/api/public/otel`. Credentials stay in Brain UI and are no longer forwarded to new Deploy Devboxes, so their Codex Langfuse tracing is disabled. When upgrading from a version that forwarded these keys, revoke the old pair and configure a fresh pair only in Brain UI: existing Devboxes retain their old environment.
