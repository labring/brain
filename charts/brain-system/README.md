...
- optional Langfuse tracing for Chat Assistant and GitHub Deploy (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_HOST`); when both keys are set, Brain exports AI SDK v7 traces and new GitHub Deploy Devboxes export Codex traces to the same instance. Leave keys empty to disable. `LANGFUSE_HOST` is the instance root URL (default `https://cloud.langfuse.com`), including for self-hosted Langfuse; do not append `/api/public/otel`.
