...
func updateAP(ctx context.Context, req apUpdateRequest) (json.RawMessage, error) {
	...
	currentServiceAnnotations, err := currentAPServiceAnnotations(cfg, *workload)
	if err != nil {
		return nil, apUpdateInternal("failed to read AP service", err)
	}
	plan, err := buildAPUpdatePlan(*workload, req.Body, currentConfigMaps, currentServiceAnnotations, time.Now().UTC())
	if err != nil {
		return nil, err
	}
...
func apUpdateMergePatch(workload apWorkload, raw json.RawMessage, currentConfigMaps []orchestration.APConfigMapMount, now time.Time) ([]byte, error) {
	...
	if metadataPatch, _ := apUpdateMetadataPatch(raw); metadataPatch != nil {
		...
		if err := applyDisplayNameAnnotationPatch(metadataPatch, annotations); err != nil {
			return nil, err
		}
	}
...
func applyDisplayNameAnnotationPatch(metadataPatch map[string]interface{}, annotations map[string]interface{}) error {
	...
	raw, ok := annotationPatch[orchestration.BrainDisplayNameAnnotation]
	if !ok {
		return nil
	}
	...
	annotations[orchestration.BrainDisplayNameAnnotation] = value
	return nil
}
