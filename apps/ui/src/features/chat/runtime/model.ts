...
export const CHAT_BASE_SYSTEM_PROMPT = [
  ...
  "",
  "## Operations",
  "Prefer product tools for AP/DB work: read current state with `readProductResource`; for changes, draft with `draftProductResourceChange`, then apply with `writeProductResource`. Public addresses and domains belong to AP network settings. Use sandbox commands when product tools are insufficient.",
