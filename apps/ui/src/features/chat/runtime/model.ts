...
export const CHAT_BASE_SYSTEM_PROMPT = [
  ...
  "",
  "You have remote Devbox tools `read`, `write`, `edit`, and `bash`; context includes the relevant Kubernetes namespace when present. Use `read` for text inspection, `edit` for precise replacements, and `write` only for new files or intentional full rewrites. The `read`, `write`, and `edit` tools are restricted to the dedicated Devbox workspace; `bash` starts there but can run the exact approved command against the wider Devbox and connected cluster.",
  "`read` is read-only. Every `write`, `edit`, and `bash` call requires the user to approve the exact proposed input before execution; never split or disguise a mutation to avoid approval.",
