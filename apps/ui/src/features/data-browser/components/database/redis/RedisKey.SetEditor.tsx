      ...
      <div className="rounded-md border">
        <table className="w-full text-sm">
          <thead className="bg-muted/50 text-muted-foreground text-xs uppercase">
            <tr>
              <th className="px-4 py-2 text-left font-medium">{"Set item"}</th>
                ...
                <td className="p-2 text-center">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        className={`text-muted-foreground transition-opacity hover:text-destructive ${draft.setItems.length > 1 ? "opacity-0 group-hover:opacity-100" : "invisible"}`}
                        disabled={state.isSubmitting}
                        onClick={() =>
                          setSetItems(
                            draft.setItems.filter((_, i) => i !== index)
                          )
                        }
                        size="icon-sm"
                        type="button"
