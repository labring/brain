...
import {
  type EnvRawSourceCompletionItem,
  EnvRawSourceEditor,
} from "./env-raw-source-editor";
import {
...
function apEnvReferenceCompletionItems(
  sources: readonly ApEnvDbDsnSource[]
): EnvRawSourceCompletionItem[] {
  return sortedAvailableReferenceItems(sources).map((item, index) => {
    const fieldOrder =
    ...
    return {
      ...
      sortText: `${String(fieldOrder).padStart(2, "0")}-${item.dbName}-${String(
        index
      ).padStart(3, "0")}`,
    };
  });
...
export function EditableEnvRows({
  ...
  if (mode === "raw") {
    editorContent = (
      <EnvRawSourceEditor
        completionItems={apEnvReferenceCompletionItems(dbDsnReferenceSources)}
