...
test("entrypoint refresh backs off while public routing waits for entrypoints", () => {
  const publicEndpointSinceByKey = new Map<string, number>();
  const apsData = {
    items: [
      {
  ...
  assert.equal(
    entryPointRefreshIntervalForLifecycle({
      apsData,
      entryPointsData: { items: [] },
      now: 100_000,
      publicEndpointSinceByKey,
      workloadReconcilePollUntil: 0,
    }),
    WORKLOAD_LIST_FAST_REFRESH_MS
  );
...
test("entrypoint refresh clears public routing age after entrypoints appear", () => {
  const publicEndpointSinceByKey = new Map<string, number>();
  const apsData = {
    items: [
      {
  ...
  assert.equal(publicEndpointSinceByKey.size, 1);
  ...
  assert.equal(publicEndpointSinceByKey.size, 0);
});
