...
describe("deployment timeout policy", () => {
  it("keeps shared task and direct apply budgets stable", () => {
    expect(DEPLOY_TIMEOUT_POLICY.overallMs).toBe(90 * MINUTE_MS);
    ...
    expect(DEPLOY_TIMEOUT_POLICY.readinessMs).toBe(30 * MINUTE_MS);
  ...
  it("keeps Agent execution, verification, and slack within the overall timeout", () => {
    ...
    expect(AGENT_DEPLOY_TIMEOUT_POLICY.verifyMs).toBe(30 * MINUTE_MS);
  ...
  it("derives the task deadline from the run start", () => {
    ...
    ).toBe(Date.parse("2026-07-27T01:30:00.000Z"));
  });
