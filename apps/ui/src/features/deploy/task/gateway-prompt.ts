...
export function buildManagedGatewayPrompt(input: {
  ...
  return [
    ...
    "Work in /home/devbox/project and run the sealos-deploy skill at /home/devbox/project/.agents/skills/sealos-deploy/SKILL.md in managed mode.",
    `Resume mode: ${input.resumeMode}`,
    "Read task identity and limits only from SEALAI_DEPLOY_TASK_ID, SEALAI_PROJECT_ID, SEALAI_DEPLOY_LABELS_PATH, SEALAI_NAMESPACE, SEALAI_INPUTS_PATH, SEALOS_REGION, SEALAI_TEMPLATE_API_URL, and SEALAI_TURN_DEADLINE_AT. Do not enumerate the environment or read/print the control capability token. Brain owns the namespace; Sealos and the Skill own the actual Instance name.",
    "Use the injected kubeconfig at $KUBECONFIG directly; the Sealos region is SEALOS_REGION and the Template API is SEALAI_TEMPLATE_API_URL. You own build, kubectl apply, observation, logs, diagnosis, repair, re-apply, and runtime verification. Brain will not execute Kubernetes mutations for you.",
    ...
    "Generate the canonical Sealos Template at /home/devbox/project/.sealos/template/index.yaml, compute its lowercase SHA-256, then call the template_ready MCP tool with only sha256. The platform provides ownership labels as a JSON object in the file at SEALAI_DEPLOY_LABELS_PATH; preserve them and let the sealos-deploy skill pass them to the Template API as extraLabels. Do not fabricate deployment-name or template-name labels, and do not use an Instance name supplied by Brain.",
    ...
    input.repairFindings?.length
      ...
      : null,
    "Only after your own deployment checks pass, call deployment_completed with the actual workload resource references you just deployed (apiVersion, kind, name, namespace). Declare every user-facing HTTP or WebSocket entry in accessEndpoints with a stable id, clear label, and exact url; use publicUrl only for v1 compatibility. Brain treats workload references as lookup targets, validates and probes every declared endpoint, and does not infer protocols or paths. If Brain returns repair, use its findings as evidence, diagnose and repair yourself, then call deployment_completed again. If Brain returns accepted_stop, end the turn successfully.",
