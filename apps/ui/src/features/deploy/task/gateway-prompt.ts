...
export function buildManagedGatewayPrompt(input: {
  ...
  return [
    ...
    "Read task identity and limits only from SEALAI_DEPLOY_TASK_ID, SEALAI_PROJECT_ID, SEALAI_NAMESPACE, SEALAI_INPUTS_PATH, and SEALAI_TURN_DEADLINE_AT. Do not enumerate the environment or read/print the control capability token. Brain owns the namespace; Sealos and the Skill own the actual Instance name.",
    ...
    "Generate the canonical Sealos Template at /home/devbox/project/.sealos/template/index.yaml, compute its lowercase SHA-256, then call the template_ready MCP tool with only sha256. Do not add Brain identity labels or extraLabels, and do not use an Instance name supplied by Brain.",
