...
The command must succeed with the exact Brain contract. In particular, `SEALAI_INPUTS_PATH` must be `/run/sealai/deployment/inputs.json`, and `SEALAI_DEPLOY_LABELS_JSON` must be valid JSON ownership labels; do not substitute a workspace path, create a second input artifact, or fabricate labels. The active kubeconfig and namespace are supplied by Brain, so managed mode skips OAuth, region/workspace selection, confirmation prompts, and system-tool installation. Use `SEALAI_NAMESPACE` as the target namespace and keep `SEALAI_TURN_DEADLINE_AT` as the hard deadline.
