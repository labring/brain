...
function parseManagedLabels(env) {
  let labels
  try {
    labels = JSON.parse(env.SEALAI_DEPLOY_LABELS_JSON)
  } catch {
    throw new Error('SEALAI_DEPLOY_LABELS_JSON must be valid JSON.')
  }
  if (!labels || typeof labels !== 'object' || Array.isArray(labels)) {
    throw new Error('SEALAI_DEPLOY_LABELS_JSON must contain a JSON object.')
  }
  for (const [key, value] of Object.entries(labels)) {
    if (!key.trim() || typeof value !== 'string') {
      throw new Error('SEALAI_DEPLOY_LABELS_JSON must contain string key/value pairs.')
    }
  }
  return labels
}
