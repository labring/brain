  ...
  withFixture((fixture) => {
    ...
    const result = runDeploy(fixture, [], {
      ...
      SEALAI_DEPLOY_LABELS_JSON: JSON.stringify({
        ...
        'brain.io/project-id': 'project-123',
...
test('forwards explicit labels to the Template API', () => {
  withFixture((fixture) => {
    const result = runDeploy(
      fixture,
      ['--labels-json', '{"brain.io/task-id":"task-123"}'],
      {
    ...
    const request = JSON.parse(readFileSync(fixture.capturePath, 'utf8'))
    assert.deepEqual(request.body.extraLabels, { 'brain.io/task-id': 'task-123' })
  })
})

test('rejects managed deployment without ownership labels', () => {
  withFixture((fixture) => {
    writeFileSync(
      join(fixture.homeDir, 'managed-kubeconfig'),
      'apiVersion: v1\nclusters:\n- cluster:\n    server: https://managed.sealos.io:6443\n',
    ...
    assert.equal(result.status, 1)
    assert.match(result.stderr, /Managed deployment requires SEALAI_DEPLOY_LABELS_JSON/)
    assert.equal(existsSync(fixture.capturePath), false)
  })
})
