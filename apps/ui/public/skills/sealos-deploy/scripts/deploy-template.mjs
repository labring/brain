 ...
 *   - Managed mode forwards SEALAI_DEPLOY_LABELS_JSON as Template API extraLabels
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--labels-json') {
      if (i + 1 >= args.length || args[i + 1].startsWith('--')) {
        fail('--labels-json requires a JSON object value')
      }
      labelsJson = args[i + 1]
      i += 1
      continue
...
function loadDeployLabels({ labelsJson }) {
  if (MANAGED_MODE && labelsJson !== null) {
    fail('Managed deployment labels must come from SEALAI_DEPLOY_LABELS_JSON')
  }
  const raw = labelsJson ?? process.env.SEALAI_DEPLOY_LABELS_JSON
  if (!raw) {
    if (MANAGED_MODE) {
      fail('Managed deployment requires SEALAI_DEPLOY_LABELS_JSON')
    }
    return {}
  }
  ...
  try {
    ...
    fail(labelsJson ? 'Failed to parse --labels-json' : 'Failed to parse SEALAI_DEPLOY_LABELS_JSON')
  }
...
async function postTemplate({ deployUrl, kubeconfig, yaml, args, dryRun, extraLabels }) {
  const response = await fetch(deployUrl, {
    ...
    body: JSON.stringify({
      ...
      args,
      dryRun,
      ...(Object.keys(extraLabels).length > 0 ? { extraLabels } : {}),
    }),
  })
...
const labelsSupplied = Object.keys(deployLabels).length
...
try {
  ...
  const payload = {
    ...
    labels_supplied: labelsSupplied,
