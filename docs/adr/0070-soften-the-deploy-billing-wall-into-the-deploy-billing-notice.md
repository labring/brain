...
**The notice speaks only of conditions that doom every deployment.** Three kinds: Account Debt on a Pay-As-You-Go workspace (top-up CTA), a full cpu, memory, or pod quota (usage CTA), and — new, settling ADR-0068's deferred follow-up — a payment-due Workspace Subscription under the Deletion Countdown (renew CTA, the Status Hint's voice beside the form). A full storage or nodeport quota leaves the pre-form surface entirely: it dooms only workloads that request those resources, so it is told by form validation on the fields that ask for them.
