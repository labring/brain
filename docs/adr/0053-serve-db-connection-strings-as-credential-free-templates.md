...
Display layer revised by ADR-0055: UI DB connection rows mask the whole value and reveal on demand. The API contract decided here — templates on read paths, the reveal endpoint, the no-credential-decoding invariant — is unchanged.
...
- **Masking the whole connection string in the API response.** Rejected at the data layer because the address is not a secret (the private address is composed deterministically from the DB name and namespace) and canvas connection derivation needs it. Display-layer masking was later adopted by ADR-0055 — the API keeps serving the template; only the UI rows render a mask.
