...
All product deployment surfaces and assistant deployment tools should create Deployment Tasks rather than directly applying resources. Direct and template runners may complete quickly, but they should still emit task events and artifact summaries so the deployment history is uniform.
