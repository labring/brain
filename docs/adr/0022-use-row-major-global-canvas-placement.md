...
- Continue opening new global placement blocks to the right of the saved layout: rejected because projects drift into long horizontal strips, and late-arriving related nodes such as AP-bound EntryPoints can permanently widen the canvas.
...
- Add only EntryPoint-specific placement exceptions: rejected because it addresses one symptom without fixing the global fallback rule that makes ordinary AP and DB creation keep widening the canvas.

## Consequences

Unanchored first placements remain deterministic and append-only: new nodes are placed without moving existing saved positions or filling arbitrary holes in the middle of a user-arranged canvas. When a new AP has desired Public Address intent, the AP and AP-bound EntryPoint are evaluated as one Canvas Placement Group with fixed internal layout and one combined footprint; placement chooses a position for that footprint, then expands it into the individual EntryPoint and AP node positions.
