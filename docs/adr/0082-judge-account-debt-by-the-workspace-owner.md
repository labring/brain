...
- Brain reads the Namespace object with the request kubeconfig for the first time. A Developer role's `get` on `*` in the namespace is expected to cover `namespaces/<own name>` (Kubernetes scopes that request to the namespace itself); this is verified during implementation, and a denial lands in the unknown-owner branch rather than an error.
